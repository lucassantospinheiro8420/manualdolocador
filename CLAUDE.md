...
**Rota de VSL nova:** adicione a linha no `MAPA` do `index.html` + crie `src/routes/<slug>.tsx` copiando `vsl-2.tsx` e trocando os 2 ids do player.

**Diagramação:** `text-balance` na tarja, headline e sub, `text-pretty` no rodapé, e `&nbsp;` amarrando as duas últimas palavras. Regra do dono: **nenhuma linha fica com uma palavra sozinha.** Medido em 360, 376 e 390px.
